...
#' 
#' \itemize{
#'     \item Tab-separated data (.tsv), using \code{\link[data.table]{fwrite}}. Setting \code{fwrite = FALSE} will rely on \code{\link[utils]{write.table}} with \code{row.names = FALSE}, for much slower export.
#'     \item Comma-separated data (.csv), using \code{\link[data.table]{fwrite}}. Setting \code{fwrite = FALSE} will rely on \code{\link[utils]{write.table}} with \code{row.names = FALSE}, for much slower export.
#'     \item \href{https://github.com/csvy}{CSVY} (CSV with a YAML metadata header) using \code{\link[csvy]{write_csvy}}. The YAML header lines are preceded by R comment symbols (\#) by default; this can be turned off by passing a \code{comment_header = FALSE} argument to \code{export}.
